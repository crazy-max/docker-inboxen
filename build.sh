...
# Wait
echo -n "Waiting for ${PROJECT} running..."
counter=1
while true; do
  if [[ ! -z $(docker ps --quiet --filter name=^/${PROJECT}$ --filter status=running) ]]; then
    break
  fi
  counter=$((counter + 1))
  echo -n "."
  sleep 1
  if [[ ${counter} -gt ${RUNNING_TIMEOUT} ]]; then
    >&2 echo -e "\nERROR: Failed to run ${PROJECT} container"
    >&2 docker logs ${PROJECT}
    exit 1
  fi;
done
echo .
...
if [ "${VERSION}" == "local" -o "${TRAVIS_PULL_REQUEST}" == "true" ]; then
  echo "This is a PR or a local build, skipping push..."
  exit 0
fi
